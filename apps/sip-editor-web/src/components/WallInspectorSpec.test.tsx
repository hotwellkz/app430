...
describe('WallInspector spec fields', () => {
  beforeEach(() => {
    useEditorStore.getState().reset();
  });

  it('shows spec summary block for selected wall', () => {
    ...
    render(<WallInspector />);
    expect(screen.getByText(/SPEC panels \/ trimmed \/ area/i)).toBeTruthy();
    expect(screen.getByText(/SIP: panel type \(override\)/i)).toBeTruthy();
  });
});
