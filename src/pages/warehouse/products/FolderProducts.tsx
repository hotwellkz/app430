    ...
    const loadFolder = async () => {
      ...
      if (folderDoc.exists()) {
        ...
        if (data.companyId === companyId) {
          setFolder({ id: folderDoc.id, ...data } as ProductFolder);
        }
      }
    };
    ...
    const unsubscribe = onSnapshot(
      ...
      (err: unknown) => {
        ...
        if (code === 'failed-precondition') {
          console.warn('[FolderProducts] Firestore index building. Empty list until ready.');
          setProducts([]);
        } else {
          console.error('FolderProducts subscription error:', err);
        }
        setLoading(false);
      }
    );
