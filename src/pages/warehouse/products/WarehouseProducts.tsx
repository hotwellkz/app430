  ...
  useEffect(() => {
    ...
    const unsubscribeProducts = onSnapshot(
      ...
      (err: unknown) => {
        ...
        if (code === 'failed-precondition') {
          console.warn('[WarehouseProducts] Firestore index building. Empty list until ready.');
          setProducts([]);
        } else {
          console.error('WarehouseProducts products subscription:', err);
        }
      }
    );
    const unsubscribeFolders = onSnapshot(
      ...
      (err: unknown) => {
        const code = (err as { code?: string })?.code;
        if (code === 'failed-precondition') {
          console.warn('[WarehouseProducts] Firestore index building. Empty folders until ready.');
          setFolders([]);
        } else {
          console.error('WarehouseProducts folders subscription:', err);
        }
        setLoading(false);
      }
    );
